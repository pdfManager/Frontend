...
function File() {
  ...
  useEffect(() => {
    ...
    const fetchFiles = async () => {
      try {
        const response = await axios.get('http://ec2-3-110-213-12.ap-south-1.compute.amazonaws.com/api/getFiles', {
          headers: {
  ...
  const deleteFile = async (fileName) => {
    ...
    try {
      await axios.delete(`http://ec2-3-110-213-12.ap-south-1.compute.amazonaws.com/api/deleteFile/${fileName}`, {
        headers: {
          authorization: `${authToken}`,
  ...
  const generateShareableLink = async (fileName) => {
    try {
      const response = await axios.post(
        'http://ec2-3-110-213-12.ap-south-1.compute.amazonaws.com/api/generateShareLink',
        { fileName },
        {
          headers: {
            authorization: `${authToken}`,
  ...
  const handleShareFile = async (fileName) => {
    try {
      ...
      await axios.post('http://ec2-3-110-213-12.ap-south-1.compute.amazonaws.com/api/share', { email, fileName, senderEmail },
        {
          headers: {
            authorization: `${authToken}`,
          },
      ...
      {selectedFile && (
        <Modal centered show={showPdfModal} onHide={handleClosePdfModal} size="lg">
          ...
          <div className=" justify-content-center">
            <Modal.Body>
              <Document file={`http://ec2-3-110-213-12.ap-south-1.compute.amazonaws.com/api/upload/${selectedFile}`} onLoadSuccess={onDocumentLoadSuccess}>
